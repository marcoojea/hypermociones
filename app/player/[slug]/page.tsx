  ...
  return (
    <AppShell active="players">
      ...
      <section className="profile-header">
        ...
        <div className="profile-title"><div><span className="position-badge">{player.position}</span><span className={`status status-${player.status.toLowerCase()}`}><i />{statusLabels[player.status]}</span></div><h1>{player.name}</h1><p>{player.team.name} · {player.nationality}{player.age !== null ? ` · ${player.age} años` : ""}{player.heightCm ? ` · ${player.heightCm} cm` : ""}{player.weightKg ? ` · ${player.weightKg} kg` : ""}</p></div>
