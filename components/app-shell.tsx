    ...
    <div className="app-frame">
      <header className="topbar">
        ...
        <nav aria-label="Navegación principal">
          {nav.map(([key, href, label]) => <Link aria-current={active === key ? "page" : undefined} className={active === key ? "active" : href === "#" ? "disabled" : ""} href={href} key={key}>{label}</Link>)}
        </nav>
        <div className="round-pill"><span>J3</span><small>2026/27</small></div>
      </header>
      <main>{children}</main>
