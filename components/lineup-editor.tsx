...
import { clampConfidence, emptyLineup, formationCodes, formations, lineupStorageKey, type FormationCode, type StoredLineup } from "@/domain/lineup";
...
function validStoredLineup(value: unknown, teamId: string, round: number): value is StoredLineup {
  if (!value || typeof value !== "object") return false;
  const lineup = value as Partial<StoredLineup>;
  return lineup.version === 1 && lineup.teamId === teamId && lineup.round === round && formationCodes.includes(lineup.formation as FormationCode) && Array.isArray(lineup.starters);
}
...
export function LineupEditor({ teams, team, players, round, fixture }: { teams: TeamSummary[]; team: TeamSummary; players: PlayerListItem[]; round: number; fixture: FixtureListItem | null }) {
  ...
  const importLineup = async (file: File | undefined) => {
    ...
    try {
      ...
      if (!validStoredLineup(parsed, team.id, round)) throw new Error("El archivo no corresponde a este equipo y jornada.");
  ...
  return <>
    <section className="editor-header"><div><p className="eyebrow">Centro editorial · Jornada {round}</p><h1>Editor de alineaciones</h1><p>{team.name}{opponent ? ` · ${fixture?.homeTeam.id === team.id ? "vs" : "@"} ${opponent.name}` : ""}</p></div><div className={`save-state ${dirty ? "dirty" : ""}`}><i /><span>{message}</span></div></section>
    ...
    <div className="editor-layout"><section className="editor-pitch-panel"><div className="editor-section-heading"><div><p className="eyebrow">Once inicial</p><h2>{lineup.formation}</h2></div><span>{lineup.starters.filter((item) => item.playerId).length}/11 seleccionados</span></div><div className="football-pitch editor-pitch"><div className="pitch-circle" /><div className="pitch-halfway" /><div className="pitch-box pitch-box-top" /><div className="pitch-box pitch-box-bottom" />{formations[lineup.formation].map((slot) => {
      const selection = lineup.starters.find((item) => item.slotId === slot.id)!;
      return <div className="pitch-editor-slot" style={{ left: `${slot.x}%`, top: `${slot.y}%` }} key={slot.id}><select aria-label={slot.label} value={selection.playerId ?? ""} onChange={(event) => update((current) => ({ ...current, starters: current.starters.map((item) => item.slotId === slot.id ? { ...item, playerId: event.target.value || null } : item) }))}><option value="">{slot.position}</option>{players.filter((player) => player.position === slot.position).map((player) => <option disabled={selectedIds.has(player.id) && selection.playerId !== player.id} value={player.id} key={player.id}>{player.name}</option>)}</select><label><span>Confianza</span><input aria-label={`Confianza ${slot.label}`} type="number" min={0} max={100} value={selection.confidence} onChange={(event) => update((current) => ({ ...current, starters: current.starters.map((item) => item.slotId === slot.id ? { ...item, confidence: clampConfidence(Number(event.target.value)) } : item) }))} /><b>%</b></label></div>;
    })}</div></section>
      ...
      <section className="editor-card"><div className="editor-section-heading"><div><p className="eyebrow">Banquillo</p><h2>Alternativas</h2></div><span>{lineup.substitutes.length}</span></div><label className="editor-field"><span>Añadir jugador</span><select value="" onChange={(event) => { const playerId = event.target.value; if (playerId) update((current) => ({ ...current, substitutes: [...current.substitutes, { playerId, confidence: 30 }] })); }}><option value="">Seleccionar…</option>{players.filter((player) => !selectedIds.has(player.id) && !benchIds.has(player.id)).map((player) => <option value={player.id} key={player.id}>{player.position} · {player.name}</option>)}</select></label><div className="bench-list">{lineup.substitutes.map((item) => <div key={item.playerId}><span><strong>{playerById.get(item.playerId)?.name}</strong><small>{playerById.get(item.playerId)?.position}</small></span><label><input aria-label={`Confianza suplente ${playerById.get(item.playerId)?.name}`} type="number" min={0} max={100} value={item.confidence} onChange={(event) => update((current) => ({ ...current, substitutes: current.substitutes.map((bench) => bench.playerId === item.playerId ? { ...bench, confidence: clampConfidence(Number(event.target.value)) } : bench) }))} />%</label><button aria-label={`Quitar ${playerById.get(item.playerId)?.name}`} type="button" onClick={() => update((current) => ({ ...current, substitutes: current.substitutes.filter((bench) => bench.playerId !== item.playerId) }))}>×</button></div>)}</div></section>
