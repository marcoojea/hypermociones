...
export function MyTeamManager({ players, teams, rounds, provenance }: { players: PlayerListItem[]; teams: TeamSummary[]; rounds: number[]; provenance: DataProvenance }) {
  ...
  return <>
    <section className="my-team-hero"><div><p className="eyebrow">{provenance.season} · Jornada {team.round}</p><h1>Mi equipo</h1><p>Construye tu plantilla y genera el mejor once posible con reglas transparentes.</p></div><div className={`save-state ${dirty ? "dirty" : ""}`}><i /><span>{message}</span></div></section>
